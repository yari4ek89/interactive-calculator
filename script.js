...
function isA4FamilySize(sizeValue, width, height) {
    const w = toNum(width);
    const h = toNum(height);
    const isA4 = (w === 210 && h === 297) || (w === 297 && h === 210);
    const is200 = (w === 200 && h === 200);
    const is297 = (w === 297 && h === 297);
    if (isA4 || is200 || is297) return true;
...
function getPiecesPerA4(sizeValue, width, height) {
    const w = toNum(width);
    const h = toNum(height);
    const isA6 = (w === 105 && h === 148) || (w === 148 && h === 105);
    const isA5 = (w === 148 && h === 210) || (w === 210 && h === 148);
    const isA4 = (w === 210 && h === 297) || (w === 297 && h === 210);
    const is100 = (w === 100 && h === 100);
    const is148 = (w === 148 && h === 148);
    const is200 = (w === 200 && h === 200);
    const is297 = (w === 297 && h === 297);

    if (isA6 || sizeValue === 'A6' || sizeValue === '105x148' || is100 || sizeValue === '100x100') return 4;
    if (isA5 || sizeValue === 'A5' || sizeValue === '148x210' || is148 || sizeValue === '148x148') return 2;
    if (isA4 || sizeValue === 'A4' || sizeValue === '210x297' || is200 || sizeValue === '200x200' || is297 || sizeValue === '297x297') return 1;
    return getPorizka(w, h, 210, 297, 1);
}
...
function punkt82() {
    ...
    const isA3Like = (sizeValue === 'A3' || sizeValue === '297x297' || (width === 297 && height === 420) || (width === 420 && height === 297) || (width === 297 && height === 297));
    const piecesPerA4 = Math.max(1, toInt(getPiecesPerA4(sizeValue, width, height), 1));
    const a4SheetsToPrint = Math.ceil(totalSheets / piecesPerA4);
    const applyCutting = !isA4FamilySize(sizeValue, width, height);
    ...
    // Без друку
    if (vnutrDruk.val() === 'no') {
        const perA3 = Math.max(1, toInt(getPorizka(width, height, 297, 420, 1), 1));
        const a3Sheets = Math.ceil(totalSheets / perA3);

        const paperRanges = PRICES?.punkt82?.A4?.no?.[vnutrMat.val()];
        const paperUnit = pickRangePrice(paperRanges, a3Sheets);
        const paperCost = paperUnit * a3Sheets;

        const applyCut = !(isA4FamilySize(sizeValue, width, height) || isA3Like);
        const cutCost = applyCut ? getPorizkaVnutrPrice(a3Sheets) : 0;

        return paperCost + cutCost;
    }
